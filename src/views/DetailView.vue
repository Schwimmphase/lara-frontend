...
<template>
    ...
    <div v-if="!detailState.loading" class="w-100 h-100">
        <detail-sidebar-component @toggle="toggle" @bigger="bigger" :open-paper="detailState.openPaper!"
                                  :bigger-list-shown="detailState.showBigger" id="detail-sidebar-component">
    ...
    <v-snackbar v-model="pdfState.snackbar" :timeout="pdfState.snackbarTimeout" id="snackbar-detail-view-error-getting-pdf">
        ...
        <template v-slot:actions>
            <v-btn color="pink" variant="text" @click="pdfState.snackbar = false"
                   id="snackbar-detail-view-close-error-getting-pdf">
                {{ $t('words.close') }}
            </v-btn>
        </template>
    </v-snackbar>
